...
// The loop is stopped when it finds the higher values between two. its not continue to check further, even the last element is the highest number
const findBusyAirport1 = () =>{
    let mostDailyFlights = airports[0].flightsPerDay;
    for (let i=1; i<airports.length; i++){
        if(airports[i].flightsPerDay > mostDailyFlights){
            mostDailyFlights = airports[i];
        }
    }
